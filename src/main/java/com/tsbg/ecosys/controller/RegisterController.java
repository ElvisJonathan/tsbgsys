...
public class RegisterController {
    ...
    @ResponseBody
    public ResultResponse register(@RequestBody EuserInfo euserInfo) {
        //初始化构造器
        ...
        int count = euserInfoService.selectisExistUserCodeByStaffCode(userCode);
        if (count>0){
        ...
        if (number>0 && deptCode.equals("GE11700800")){
            //可以注册
            //设置创建人和创建时间
            euserInfo.setCreater(euserInfo.getUserName());
            euserInfo.setCreateTime(new Date());
            //调用业务逻辑存储注册信息
            int num = euserInfoService.insertSelective(euserInfo);
            //成功注册则会增加一条记录
            if (num>0){
                //同步存储对应信息至euser_area和euser_role
                //将euser_info表的uid、aid查询出来
                int uid = euserInfoService.selectuidbyuserCode(userCode);
                int aid = euserInfo.getAid();
                String name = euserInfo.getUserName()+"是生态员工";
                //将此uid和aid增加至euser_area
                euserInfoService.insertDatatoEuserArea(uid,aid,name);
                //将此uid和rid增加至euser_role
                euserInfoService.insertDatatoEuserRole(uid,2,name);
                //返回成功码0并且提示成功注册
